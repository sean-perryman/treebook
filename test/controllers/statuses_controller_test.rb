...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should create status for the current user when logged in" do
    sign_in users(:sean)
    assert_difference('Status.count') do
      post :create, status: { content: @status.content, user_id: users(:pimpy).id  }
    end
  ...
  test "should get edit only if logged in" do
    get :edit, id: @status
    assert_response :redirect
    assert_redirected_to new_user_session_path
  end
  ...
  test "should update status for the current user when logged in" do
    sign_in users(:sean)
    patch :update, id: @status, status: { content: @status.content, user_id: users(:pimpy).id }
    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:sean).id
  end
